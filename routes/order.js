...
router.patch('/update-order-by-id/:orderId', async (req, res) => {
  try {
    ...
    const { items, subtotal, CGST, SGST, total, isTemporary, acPercentageAmount, isPrint, cashAmount, onlinePaymentAmount, dueAmount, complimentaryAmount, discount,customerName,mobileNumber } = req.body;
...
router.get('/menu-statistics', async (req, res) => {
  try {
    ...
    // Fetch menu prices
    const menuPrices = await Menu.find({}, { name: 1, price: 1 }); // Assuming your Menu model has a 'name' and 'price' field
